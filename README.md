...
>`subs` is a globby array of `assembly.json` files that are assembled into the component output file. _Again, the name of these `JSON` files does not need to be `assembly.json`, but they must conform with a correctly formated `assembly.json` file._
